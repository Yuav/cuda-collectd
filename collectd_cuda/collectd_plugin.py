...
def read(data=None):
        ...
        for gpu in root.getiterator('gpu'):
                ...
                vl.dispatch(type='percent', type_instance='gpu_util',
                            values=[normalize(gpu.find('utilization/gpu_util').text.split()[0])])
                # GPU temperature
                vl.dispatch(type='temperature',
                            values=[normalize(gpu.find('temperature/gpu_temp').text.split()[0])])
                # GPU power draw
                vl.dispatch(type='power', type_instance='power_draw',
                            values=[normalize(gpu.find('power_readings/power_draw').text.split()[0])])
                # GPU memory utilization
                vl.dispatch(type='percent', type_instance='mem_util',
                            values=[normalize(gpu.find('utilization/memory_util').text.split()[0])])
                # GPU encoder utilization
                vl.dispatch(type='percent', type_instance='enc_util',
                            values=[normalize(gpu.find('utilization/encoder_util').text.split()[0])])
                # GPU decoder utilization
                vl.dispatch(type='percent', type_instance='dec_util',
                            values=[normalize(gpu.find('utilization/decoder_util').text.split()[0])])
                # GPU memory usage
                vl.dispatch(type='memory', type_instance='used',
                            values=[1e6 * normalize(gpu.find('fb_memory_usage/used').text.split()[0])])
                # GPU total memory
                vl.dispatch(type='memory', type_instance='total',
                            values=[1e6 * normalize(gpu.find('fb_memory_usage/total').text.split()[0])])
                # GPU frequency
                vl.dispatch(type='cpufreq', type_instance='gpu_clock',
                            values=[1e6 * normalize(gpu.find('clocks/graphics_clock').text.split()[0])])
                # GPU memory frequency
                vl.dispatch(type='cpufreq', type_instance='mem_clock',
                            values=[1e6 * normalize(gpu.find('clocks/mem_clock').text.split()[0])])
